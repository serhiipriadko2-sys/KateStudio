...
const IntroSplash = ({ onComplete }: { onComplete: () => void }) => {
  ...
  return (
    ...
    >
      ...
      <div className={`absolute bottom-24 left-0 right-0 text-center pointer-events-none px-4 h-16 transition-opacity duration-500 ${isIgnited ? 'opacity-0' : 'opacity-100'}`}>
         <div className={`transition-opacity duration-300 ${progress > 5 ? 'opacity-100' : 'opacity-40'}`}>
      ...
      {!isIgnited && progress > 0 && (
        <div className="absolute bottom-0 left-0 h-1.5 bg-[#FCEEAC] transition-all duration-75 ease-linear shadow-[0_0_20px_#FCEEAC]" style={{ width: `${progress}%` }}></div>
      )}
    </div>
  );
};
...
export default function App() {
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-[#FDFBF7] text-brand-text overflow-hidden relative selection:bg-brand-green selection:text-white animate-in fade-in duration-1000"
         onContextMenu={(e) => e.preventDefault()}>
      
      <header className={`absolute top-0 left-0 right-0 z-30 pt-safe px-6 pb-4 transition-all duration-300 flex justify-between items-center pointer-events-none ${scrolled ? 'bg-white/90 backdrop-blur-md shadow-sm' : 'bg-transparent'}`}>
        <div className="pointer-events-auto flex items-center gap-3">
           ...
           <span className={`font-serif text-lg font-medium transition-opacity duration-300 ${scrolled ? 'opacity-100 text-brand-text' : 'opacity-0'}`}>
             К себе
           </span>
        </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-xl border-t border-stone-200/50 pb-safe z-40 shadow-[0_-5px_30px_rgba(0,0,0,0.03)]">
        <div className="flex justify-around items-end h-16 px-2">
          <NavButton active={activeTab === 'home'} onClick={() => handleTabChange('home')} icon={<Home />} label="Главная" />
          <NavButton active={activeTab === 'schedule'} onClick={() => handleTabChange('schedule')} icon={<Calendar />} label="Расписание" />
          ...
          <NavButton active={activeTab === 'studio'} onClick={() => handleTabChange('studio')} icon={<MapPin />} label="Студия" />
          <NavButton active={false} onClick={() => handleTabChange('profile')} icon={<User />} label="Профиль" />
        </div>
      </nav>
...
const NavButton = ({ active, onClick, icon, label }: { active: boolean, onClick: () => void, icon: React.ReactNode, label: string }) => (
  ...
  >
    ...
    <span className={`text-[10px] font-bold tracking-wide transition-all duration-300 ${active ? 'opacity-100' : 'opacity-0 hidden'}`}>
      {label}
    </span>
  </button>
);
...
const HomeView = ({ setActiveTab }: { setActiveTab: (t: Tab) => void }) => {
  ...
  const getAmbientColor = () => {
    switch (selectedMood) {
      case "Энергия": return "bg-amber-400/10";
      case "Покой": return "bg-indigo-400/10";
      case "Здоровье": return "bg-rose-400/10";
      case "Сила": return "bg-emerald-400/10";
  ...
  return (
    <div className="view-transition relative">
      ...
      <div className={`fixed inset-0 pointer-events-none transition-colors duration-1000 z-0 ${getAmbientColor()}`}></div>
      ...
      <div className="relative z-10">
        <div className="relative h-[60vh] min-h-[500px] w-full overflow-hidden rounded-b-[3rem] shadow-2xl">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-8 text-white z-10 pointer-events-none">
             <div className="pointer-events-auto">
               <h3 className="text-sm font-bold uppercase tracking-widest mb-2 opacity-80 animate-in fade-in slide-in-from-bottom-2">Доброе утро</h3>
               <h1 className="text-5xl font-serif mb-6 leading-tight animate-in fade-in slide-in-from-bottom-4 delay-100">Время<br/>выбрать себя</h1>
               <div className="flex gap-4 animate-in fade-in slide-in-from-bottom-6 delay-200">
                 <button onClick={() => setActiveTab('schedule')} className="flex-1 bg-white text-brand-dark py-4 rounded-2xl font-bold uppercase text-xs tracking-wider hover:bg-brand-mint transition-colors shadow-lg">
                   Расписание
                 </button>
                 <button onClick={() => setActiveTab('ai')} className="w-14 h-14 bg-white/20 backdrop-blur-md border border-white/30 rounded-2xl flex items-center justify-center hover:bg-white/30 transition-colors shadow-lg">
        ...
        <div className="mt-12 mb-6">
          <div className="px-6 mb-2 flex justify-between items-end">
            ...
            {selectedMood && (
               <button onClick={() => setSelectedMood(null)} className="text-xs text-stone-400 flex items-center gap-1 hover:text-rose-500 transition-colors">
          ...
          <div className="flex overflow-x-auto px-6 gap-4 pb-8 scrollbar-hide snap-x">
            {[
              { icon: <Zap />, title: "Энергия", color: "bg-amber-100 text-amber-600", activeColor: "bg-amber-500 text-white shadow-lg shadow-amber-500/30 ring-2 ring-amber-200" },
              { icon: <Moon />, title: "Покой", color: "bg-indigo-100 text-indigo-600", activeColor: "bg-indigo-500 text-white shadow-lg shadow-indigo-500/30 ring-2 ring-indigo-200" },
              { icon: <Heart />, title: "Здоровье", color: "bg-rose-100 text-rose-600", activeColor: "bg-rose-500 text-white shadow-lg shadow-rose-500/30 ring-2 ring-rose-200" },
              { icon: <Activity />, title: "Сила", color: "bg-emerald-100 text-emerald-600", activeColor: "bg-emerald-500 text-white shadow-lg shadow-emerald-500/30 ring-2 ring-emerald-200" },
            ].map((item, i) => {
              ...
              return (
                ...
                >
                   ...
                   <span className={`text-xs font-medium transition-colors ${isActive ? 'text-brand-text font-bold' : 'text-stone-500'}`}>{item.title}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="px-4 mb-8 mt-12">
           <div className="bg-[#1a1a1a] rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-xl group">
              ...
              <div className="relative z-10 pointer-events-none">
                 <span className="px-3 py-1 bg-brand-green rounded-full text-[10px] font-bold uppercase mb-4 inline-block">Новое</span>
                 <h3 className="text-3xl font-serif mb-2">Inside Flow</h3>
                 <p className="text-white/60 text-sm mb-6 max-w-[200px]">Танцуй своим дыханием под современную музыку.</p>
                 <div 
                   onClick={() => setActiveTab('studio')} 
