...
export const Schedule: React.FC = () => {
  ...
  const renderCalendar = () => {
    ...
    for (let i = 1; i <= slotsToFill && i <= 14; i++) {
      // Limit to avoid huge empty grids
    ...
    <section id="schedule" className="py-12 md:py-24 px-4 md:px-12 max-w-7xl mx-auto scroll-mt-20">
      <div className="text-center mb-12">
        <FadeIn>
          ...
          <p className="text-stone-500 max-w-2xl mx-auto">Выберите удобный день в календаре.</p>
        </FadeIn>
      </div>
            ...
            >
              <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="p-2 hover:bg-stone-50 rounded-full transition-colors active:scale-90"
                  >
                    <ChevronLeft className="w-5 h-5 text-stone-400" />
                  </button>
                  <span className="text-lg font-serif capitalize text-brand-text min-w-[120px] text-center select-none">
              ...
              <div className="grid grid-cols-7 gap-1 text-center mb-4">
                {['Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб', 'Вс'].map((d) => (
                  <div key={d} className="text-xs text-stone-400 font-bold uppercase">
                    {d}
                  </div>
                ))}
              </div>
          ...
          <FadeIn delay={300} direction="left">
            <div className="mb-6 flex items-end gap-4 pl-2">
              <h3 className="text-4xl md:text-5xl font-serif text-brand-text capitalize">
                {selectedDateNum} {currentMonth.toLocaleString('ru', { month: 'long' })}
              </h3>
                          ...
                          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                            <div className="flex items-start gap-6">
                              <div className="flex flex-col items-center min-w-[60px]">
                                ...
                                <span className="text-xs text-stone-400 bg-stone-50 px-2 py-1 rounded-md mt-1">
                                  {cls.duration}
                                </span>
                              </div>
                            ...
                            <div className="flex flex-col justify-end gap-4 md:w-auto w-full">
                              <div className="flex flex-col w-full md:w-32">
                                ...
                                <div className="h-1.5 w-full bg-stone-100 rounded-full overflow-hidden">
                                  <div
                                    className={`h-full rounded-full transition-all duration-500 ${isFull ? 'bg-stone-300' : statusColor}`}
