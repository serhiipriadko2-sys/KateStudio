        ...
        <div className="bg-[#1a1a1a] text-white rounded-[3rem] p-8 md:p-16 relative overflow-hidden flex flex-col justify-center">
          ...
          <div className="relative z-10">
            <FadeIn>
              ...
              <p className="text-white/50 mb-10 font-light text-lg">Оставьте заявку, и мы перезвоним в течение 15 минут</p>
            </FadeIn>
            
            <form className="space-y-6" onSubmit={handleSubmit}>
              <FadeIn delay={100} fullWidth>
                <input type="text" placeholder="Ваше имя" required className="w-full bg-transparent border-b border-white/20 text-white px-4 py-4 focus:outline-none focus:border-brand-green placeholder:text-white/30 transition-all text-lg" />
              </FadeIn>
              <FadeIn delay={200} fullWidth>
                <input type="tel" placeholder="Телефон" required className="w-full bg-transparent border-b border-white/20 text-white px-4 py-4 focus:outline-none focus:border-brand-green placeholder:text-white/30 transition-all text-lg" />
              </FadeIn>
              <FadeIn delay={300} fullWidth>
                <textarea rows={2} placeholder="Ваш вопрос" className="w-full bg-transparent border-b border-white/20 text-white px-4 py-4 focus:outline-none focus:border-brand-green placeholder:text-white/30 transition-all resize-none text-lg" />
              </FadeIn>
              <FadeIn delay={400} fullWidth>
                <button 
                    ...
                    `}
                >
                    {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : sent ? <Check className="w-5 h-5" /> : 'Отправить'}
                </button>
                <p className="text-[10px] text-white/20 text-center px-4 pt-4">Нажимая кнопку, вы соглашаетесь с условиями обработки данных</p>
              </FadeIn>
            </form>
          </div>
        </div>
                ...
                <div className="absolute bottom-8 left-8 right-8">
                   ...
                   >
                      <div className="flex items-center justify-between">
                         <div>
                            <p className="text-xs text-stone-400 uppercase tracking-wider font-bold mb-1">Студия на Мира</p>
