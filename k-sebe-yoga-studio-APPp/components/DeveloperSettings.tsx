...
import { Save, RotateCcw, Image as ImageIcon, Palette, Upload, Check, AlertTriangle, Download, FileJson, RefreshCw } from 'lucide-react';
...
export const DeveloperSettings: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const handleExportBackup = () => {
    ...
    for(let i=0; i<localStorage.length; i++) {
        ...
        if(key && (key.startsWith('gallery-') || key.startsWith('review-')) && !backupData.assets[key]) {
  ...
  return (
    <div className="h-full flex flex-col bg-[#111] text-white overflow-hidden">
      ...
      <div className="p-6 border-b border-white/10 flex justify-between items-center bg-[#111] z-10">
        ...
        <button onClick={onBack} className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-lg text-xs font-mono transition-colors">
          EXIT
        </button>
      </div>
        ...
        {activeTab === 'colors' && (
          <div className="max-w-2xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4">
            <div className="bg-white/5 rounded-xl p-6 border border-white/10">
              <h3 className="font-mono text-sm text-stone-400 mb-6 uppercase tracking-widest border-b border-white/5 pb-2">Глобальные переменные</h3>
          ...
          <div className="max-w-3xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4">
            ...
            <div className="grid gap-4">
               {ASSET_SLOTS.map((slot) => {
                 ...
                 return (
                   <div key={slot.key} className="bg-white/5 rounded-xl p-4 border border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div className="flex items-start gap-4">
                         <div className="w-12 h-12 bg-white/5 rounded-lg flex items-center justify-center text-stone-600 font-mono text-xs border border-white/5 shrink-0 overflow-hidden relative group">
                            ...
                            {localStorage.getItem(slot.key) ? (
                                <img src={localStorage.getItem(slot.key)!} className="w-full h-full object-cover" />
                            ) : 'IMG'}
                         </div>
                         <div>
                            <h4 className="font-bold text-sm text-stone-200">{slot.label}</h4>
                            <code className="text-[10px] text-brand-green bg-brand-green/10 px-1.5 py-0.5 rounded block w-fit mt-1 mb-1">{slot.key}</code>
                            <p className="text-[10px] text-stone-500">{slot.desc}</p>
                         </div>
                      </div>

                      <div className="flex items-center gap-3">
                         {status === 'success' && <span className="text-xs text-green-400 font-mono flex items-center gap-1"><Check className="w-3 h-3"/> Saved</span>}
        ...
        {activeTab === 'backup' && (
          <div className="max-w-2xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4">
             ...
             <div className="bg-stone-900 rounded-xl p-4 border border-white/5">
                <h5 className="font-mono text-xs text-stone-500 mb-2 uppercase">Техническая информация</h5>
                <div className="font-mono text-[10px] text-stone-600 space-y-1">
                   <p>LocalStorage Usage: ~{(JSON.stringify(localStorage).length / 1024 / 1024).toFixed(2)} MB</p>
