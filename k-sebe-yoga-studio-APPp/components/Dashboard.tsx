...
import { LogOut, LayoutDashboard, Video, Wind, Calendar, Trophy, ChevronRight, User, Sparkles, Loader2, X, Edit2, Save, Terminal, Ticket, QrCode, Camera } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ onBack, initialTab = 'overview' }) => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'videos' | 'breath' | 'ai' | 'profile' | 'dev'>(initialTab);
  ...
  return (
    <div className="h-[100dvh] bg-[#FDFBF7] flex overflow-hidden">
            ...
            {activeTab === 'overview' && (
              <div className="max-w-4xl mx-auto">
                <header className="flex justify-between items-end mb-8 md:mb-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <div>
                    <h1 className="text-2xl md:text-4xl font-serif text-brand-text mb-2">Привет, {userName}!</h1>
                    <p className="text-stone-400 font-light text-sm md:text-base">Рады видеть тебя снова.</p>
                  </div>
                </header>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-8">
                   {/* Card 1 */}
                   <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-stone-100 flex flex-col justify-between h-40 md:h-48 relative overflow-hidden group animate-in zoom-in-95 duration-500 fill-mode-backwards" style={{ animationDelay: '100ms' }}>
                      ...
                      <div className="relative z-10">
                         ...
                         <p className="text-xs text-stone-400 font-medium uppercase tracking-wider">Всего записей</p>
                      </div>
                   </div>
                   
                   {/* Card 2 */}
                   <div className="bg-brand-dark p-6 rounded-[2rem] shadow-xl shadow-stone-200 flex flex-col justify-between h-40 md:h-48 text-white relative overflow-hidden animate-in zoom-in-95 duration-500 fill-mode-backwards" style={{ animationDelay: '200ms' }}>
                      ...
                      <div>
                         ...
                         <p className="text-xs text-white/50 font-medium uppercase tracking-wider">Ваш статус</p>
                      </div>
                   </div>

                   {/* Card 3 (Next Class) */}
                   <div className="bg-brand-green p-6 rounded-[2rem] shadow-xl shadow-brand-green/20 flex flex-col justify-between h-40 md:h-48 text-white relative overflow-hidden cursor-pointer group hover:scale-[1.02] transition-transform animate-in zoom-in-95 duration-500 fill-mode-backwards" style={{ animationDelay: '300ms' }}>
                         ...
                         {nextBooking ? (
                           <>
                             <div className="flex items-center gap-2 mb-4">
                               <span className="inline-block px-3 py-1 bg-white/20 backdrop-blur-md rounded-full text-[10px] font-bold uppercase tracking-wider">Ближайшее</span>
                             ...
                             <p className="text-white/70 text-sm">{nextBooking.time} • {nextBooking.date}</p>
                           </>
                         ) : (
                           <>
                              <span className="inline-block px-3 py-1 bg-white/20 backdrop-blur-md rounded-full text-[10px] font-bold uppercase tracking-wider mb-4">Нет записей</span>
                ...
                <div className="mb-24 md:mb-0 animate-in fade-in slide-in-from-bottom-8 duration-700" style={{ animationDelay: '400ms' }}>
                   ...
                   {loading ? (
                     ...
                     <div className="grid gap-4">
                         {bookings.map((b) => {
                             ...
                             return (
                             <div key={b.id} className={`relative bg-white rounded-[2rem] border transition-all duration-300 overflow-hidden flex flex-col md:flex-row ${isFuture ? 'border-brand-green/30 shadow-md hover:shadow-lg' : 'border-stone-100 opacity-80 grayscale hover:grayscale-0'}`}>
                                 {/* Left: Content */}
                                 <div className="flex-1 p-6 md:p-8 flex flex-col justify-center relative z-10">
                                     <div className="flex justify-between items-start mb-4">
                                         <div>
                                             <span className={`text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded-md mb-2 inline-block ${isFuture ? 'bg-brand-mint text-brand-green' : 'bg-stone-100 text-stone-400'}`}>
                                                 {isFuture ? 'Активен' : 'Завершен'}
                                             </span>
                                             <h4 className="text-2xl font-serif text-brand-text">{b.className}</h4>
                                         </div>
                                     ...
                                     <div className="flex items-center justify-between border-t border-dashed border-stone-200 pt-4 mt-2">
                                         <div className="flex items-center gap-2 text-sm text-stone-500">
                                             <div className="p-1.5 bg-stone-50 rounded-full"><LayoutDashboard className="w-4 h-4"/></div>
                                             {b.location}
                                         </div>
                                         {isFuture && (
                                             <button 
                ...
                <div className="max-w-md mx-auto animate-in fade-in slide-in-from-bottom-8 duration-500 pb-24">
                    <div className="flex flex-col items-center mb-8 relative">
                        ...
                        >
                            ...
                            <Image 
                                src={user?.avatar || "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=400&h=400&fit=crop"}
                                alt="User"
                        ...
                        {!isEditingProfile ? (
                            ...
                            <div className="w-full space-y-2 mt-2">
                                <p className="text-xs text-stone-400 text-center mb-2">Нажмите на фото, чтобы изменить</p>
                                <input 
                                    type="text" 
                                    value={editName} 
                                ...
                                <div className="flex justify-center gap-2 mt-2">
                                    <button onClick={() => setIsEditingProfile(false)} className="p-2 text-stone-400"><X className="w-5 h-5"/></button>
                                    <button onClick={handleSaveProfile} className="p-2 text-brand-green"><Save className="w-5 h-5"/></button>
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="bg-white rounded-[2rem] p-2 shadow-sm border border-stone-100 divide-y divide-stone-50">
                        ...
                        <button onClick={() => setActiveTab('dev')} className="w-full flex items-center justify-between p-5 hover:bg-stone-50 transition-colors rounded-xl group">
                    ...
                    <button onClick={handleLogout} className="w-full mt-6 py-4 text-rose-500 font-medium text-sm bg-rose-50 rounded-2xl hover:bg-rose-100 transition-colors">
                        Выйти из аккаунта
                    </button>
                </div>
            )}
      ...
      {/* QR Modal */}
      {expandedQr && (
          <div className="fixed inset-0 z-50 bg-black/90 flex flex-col items-center justify-center p-8 animate-in fade-in" onClick={() => setExpandedQr(null)}>
              ...
              <button onClick={() => setExpandedQr(null)} className="mt-8 p-4 bg-white/10 rounded-full text-white hover:bg-white/20 transition-colors">
                  <X className="w-6 h-6" />
              </button>
          </div>
      )}
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={`text-[9px] font-bold tracking-wide transition-opacity duration-300 ${isActive ? 'opacity-100' : 'opacity-0 absolute -bottom-2'}`}>
                {item.label}
              </span>
              {isActive && (
