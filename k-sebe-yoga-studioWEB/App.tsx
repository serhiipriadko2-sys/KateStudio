...
function App() {
  ...
  const [bookingModalData, setBookingModalData] = useState<{isOpen: boolean, details: BookingDetails}>({
    isOpen: false,
    details: { type: '' }
  });
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.ctrlKey && e.shiftKey && (e.key === 'Y' || e.key === 'y' || e.key === 'H' || e.key === 'h')) {
        e.preventDefault();
  ...
  return (
    <>
      ...
      <div className={`min-h-screen bg-brand-light font-sans selection:bg-brand-green selection:text-white relative flex flex-col transition-opacity duration-700 ${loading ? 'opacity-0' : 'opacity-100'}`}>
        {/* Texture Overlay */}
        <div 
        ...
        >
          ...
          >
            {isMenuOpen ? (
              <X className={`w-6 h-6 transition-colors ${isScrolled ? 'text-stone-800' : 'text-brand-dark md:text-white'}`} />
            ) : (
              <Menu className={`w-6 h-6 transition-colors ${isScrolled ? 'text-brand-green' : 'text-white'}`} />
            )}
          </button>
        </nav>
        ...
        {isMenuOpen && (
          ...
          >
            ...
            <nav className="flex flex-col items-center space-y-6 md:space-y-8 text-center">
              {['Обо мне', 'Направления', 'Галерея', 'Стоимость', 'Расписание', 'Блог', 'Отзывы', 'Контакты'].map((item, i) => (
                  <a 
                      key={item}
                      ...
                      style={{ animation: `fade-in-up 0.5s ease-out ${i * 0.1}s forwards`, opacity: 0, transform: 'translateY(20px)' }}
                  >
                      {item}
                  </a>
              ))}
            ...
            <div className="absolute bottom-12 flex gap-6" style={{ animation: 'fade-in 0.5s ease-out 0.8s forwards', opacity: 0 }}>
              <a href="#" className="bg-brand-mint/50 p-4 rounded-full hover:bg-brand-mint transition-colors"><Instagram className="w-6 h-6 text-brand-green" /></a>
              <a href="#" className="bg-brand-mint/50 p-4 rounded-full hover:bg-brand-mint transition-colors"><Send className="w-6 h-6 text-brand-green" /></a>
            </div>
