...
import { GoogleGenAI, Chat, GenerateContentResponse, Modality, HarmCategory, HarmBlockThreshold, Type } from "@google/genai";
import { Source, AsanaAnalysis } from "../types";

let chatSession: Chat | null = null;
let coachSession: Chat | null = null;
...
const KNOWLEDGE_BASE = {
  identity: {
    ...
    core_message: "Йога — это не про то, чтобы дотянуться руками до пальцев ног, а про то, что мы узнаем на пути вниз.",
    philosophy: "Slow life в мегаполисе. Мы учимся замедляться, чтобы услышать себя. Практика на коврике — это репетиция жизни.",
  },
...
export const getGeminiChatResponse = async (userMessage: string, location?: { lat: number, lng: number }): Promise<{text: string, sources: Source[]}> => {
  ...
  try {
    ...
    const toolConfig = location ? { toolConfig: { retrievalConfig: { latLng: { latitude: location.lat, longitude: location.lng } } } } : {};
...
export const transcribeDiaryEntry = async (audioBase64: string): Promise<{text: string, summary: string}> => {
...
export const generateMeditationScript = async (topic: string, duration: 'short' | 'medium'): Promise<string> => {
    if (!process.env.API_KEY) return "Ошибка ключа";
    try {
        const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
        const response = await ai.models.generateContent({
...
export const analyzeImageContent = async (base64Image: string): Promise<AsanaAnalysis | string> => {
  ...
  try {
    ...
    const response = await ai.models.generateContent({
      model: "gemini-2.5-flash",
      contents: { parts: [{ inlineData: { mimeType: 'image/jpeg', data: cleanBase64 } }, { text: "Analyze yoga pose." }] },
      config: { responseMimeType: "application/json", responseSchema: { type: Type.OBJECT, properties: { sanskrit: {type: Type.STRING}, name_ru: {type: Type.STRING}, muscles: {type: Type.ARRAY, items:{type:Type.STRING}}, energy: {type: Type.STRING, enum:["Brahmana", "Langhana", "Samana"]}, description: {type: Type.STRING}, tips: {type: Type.STRING} } } }
    });
    if (response.text) return JSON.parse(response.text) as AsanaAnalysis;
    return "Не удалось распознать образ.";
  } catch (error) { return "Ошибка анализа."; }
};
