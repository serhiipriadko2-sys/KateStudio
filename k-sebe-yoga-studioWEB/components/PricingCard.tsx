  ...
  return (
    ...
    `}>
      ...
      <div className="relative z-10 h-full flex flex-col justify-between">
        <div>
          <h3 className={`text-xl font-serif mb-2 ${item.isPersonal ? 'text-brand-accent' : 'text-brand-green'}`}>
            {item.title}
          </h3>
          {item.subtitle && (
            <p className={`text-sm mb-4 ${item.isPersonal ? 'text-brand-light/70' : 'text-stone-500'}`}>
              {item.subtitle}
            </p>
          )}
        </div>
