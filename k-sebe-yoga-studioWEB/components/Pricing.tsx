...
const PricingCard: React.FC<PricingCardProps> = ({ option, onBook, delay }) => (
  <FadeIn delay={delay} direction="up" className="h-full">
    <div
      className={`h-full relative flex flex-col p-5 md:p-6 rounded-[2rem] transition-all duration-500 group ${option.isPopular ? 'border-2 border-brand-green shadow-2xl bg-white lg:-mt-4 lg:mb-3 lg:py-8 z-10 hover:shadow-brand-green/20' : ''} ${option.isDark ? 'bg-[#1a1a1a] text-white shadow-xl hover:shadow-2xl hover:shadow-black/20' : !option.isPopular ? 'bg-white border border-stone-100 hover:border-brand-green/30 hover:shadow-xl' : ''}`}
    >
      {option.isPopular && (
      ...
      <div className="mb-4">
        <h3
          className={`text-base md:text-lg font-serif mb-2 group-hover:translate-x-1 transition-transform ${option.isDark ? 'text-white' : 'text-brand-text'}`}
        >
          {option.title}
        </h3>
      ...
      <div className="mb-5 relative">
        <span
          className={`text-2xl md:text-3xl font-serif tracking-tight ${option.isDark ? 'text-brand-green' : 'text-brand-text'}`}
        >
          {option.price}
        </span>
        {option.isPopular && (
      ...
      <ul className="space-y-2 mb-6 flex-1">
        {option.features.map((feature, fIdx) => (
          <li key={fIdx} className="flex items-start gap-3 text-xs md:text-sm group/item">
            <div
      ...
      <button
        ...
        className={`w-full py-2.5 rounded-xl font-medium transition-all duration-300 text-xs md:text-sm tracking-wide uppercase flex items-center justify-center gap-2 group/btn relative overflow-hidden ${option.isDark ? 'bg-brand-green text-white hover:bg-brand-green/90' : option.isPopular ? 'bg-brand-green text-white hover:bg-brand-green/90 shadow-lg shadow-brand-green/30' : 'bg-stone-50 text-brand-text hover:bg-stone-100 hover:text-brand-green'}`}
      >
...
const PricingSection: React.FC<PricingSectionProps> = ({
  ...
  return (
    <div className="mb-10 md:mb-12 last:mb-0">
      <FadeIn>
        <h3 className="text-xl md:text-2xl font-serif text-brand-text/90 mb-2 text-center">
          {title}
        </h3>
        {subtitle && <p className="text-stone-400 text-sm text-center mb-4 md:mb-6">{subtitle}</p>}
      </FadeIn>
      <div
        className={`grid grid-cols-1 ${gridCols[columns]} gap-3 md:gap-4 items-stretch mt-5 md:mt-6`}
      >
...
export const Pricing: React.FC<PricingProps> = ({ onBook }) => {
  return (
    <section id="pricing" className="py-12 md:py-16 px-4 md:px-12 max-w-7xl mx-auto scroll-mt-20">
