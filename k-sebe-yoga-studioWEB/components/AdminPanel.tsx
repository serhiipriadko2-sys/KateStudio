...
import { X, Settings, Image as ImageIcon, Palette, Save, RotateCcw, Upload, Loader2, Database, CheckCircle, AlertCircle, Download, FileJson, Trash2 } from 'lucide-react';
...
export const AdminPanel: React.FC<AdminPanelProps> = ({ isOpen, onClose }) => {
  ...
  const handleClearCache = () => {
    if (confirm("Это удалит все локально сохраненные изображения (Base64) из браузера. Если они не были загружены в облако, они пропадут. Продолжить?")) {
  ...
  const handleExportConfig = () => {
    try {
        const config = {
            theme: colors,
            images: IMAGE_REGISTRY.reduce((acc, item) => {
        ...
        downloadAnchorNode.setAttribute("download", `ksebe_config_${new Date().toISOString().slice(0,10)}.json`);
    ...
    <div className="fixed inset-0 z-[100] flex bg-stone-900/50 backdrop-blur-sm animate-in fade-in">
      <div className="w-full max-w-2xl bg-white shadow-2xl h-full ml-auto flex flex-col animate-in slide-in-from-right duration-300 relative">
        ...
        <div className="p-6 border-b border-stone-100 flex items-center justify-between bg-stone-50">
          ...
          <button onClick={onClose} aria-label="Close" className="p-2 hover:bg-stone-200 rounded-full transition-colors">
            ...
            <div className="space-y-6">
                 ...
                 <div className="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-stone-500 mt-1 max-w-sm">
                            Здесь вы можете загрузить изображения. Если облако недоступно, файлы сохранятся локально (base64).
                        </p>
                    </div>
                 </div>
                 <button 
              ...
              <div className="grid gap-4">
                {IMAGE_REGISTRY.map((item) => (
                  <div key={item.key} className="bg-white p-4 rounded-xl border border-stone-200 shadow-sm flex items-center gap-4 transition-all hover:border-brand-green/30">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <code className="text-xs text-stone-400 bg-stone-50 px-1.5 py-0.5 rounded">{item.key}</code>
                    </div>
                    <div>
                      <label className={`cursor-pointer flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${uploadingKey === item.key ? 'bg-stone-100 text-stone-400 cursor-wait' : 'bg-brand-mint/50 text-brand-green hover:bg-brand-green hover:text-white'}`}>
                        {uploadingKey === item.key ? (
                          <Loader2 className="w-4 h-4 animate-spin" />
                        ) : (
                          <Upload className="w-4 h-4" />
                        )}
                        <span className="hidden sm:inline">{uploadingKey === item.key ? '...' : 'Загрузить'}</span>
                        <input 
                          type="file" 
                          className="hidden" 
                          accept="image/*"
              ...
              <div className="space-y-6">
                 <div className="bg-white p-6 rounded-xl border border-stone-200 shadow-sm">
                    ...
                    <p className="text-stone-500 mb-6 text-sm">
                        Вы можете скачать все текущие настройки (цветовую схему и ссылки на изображения) в виде JSON файла. 
                        Это полезно для переноса настроек на другое устройство или для восстановления после очистки кэша.
                    </p>
                 ...
                 <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 flex gap-3">
                    ...
                    <div className="text-sm text-blue-800">
                        ...
                        <p>Файл содержит JSON объект с ключами <code>theme</code> и <code>images</code>. Не редактируйте его вручную, если не уверены в структуре.</p>
                    </div>
                 </div>
              </div>
          )}
...
const ColorInput: React.FC<{ label: string, variable: string, value: string, onChange: (k: any, v: string) => void }> = ({ label, variable, value, onChange }) => (
