  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const id = requestAnimationFrame(() => {
        if (parallaxRef.current) parallaxRef.current.style.transform = `translate3d(0, ${scrollY * 0.5}px, 0)`;
    ...
    return () => { window.removeEventListener('scroll', handleScroll); requestIds.forEach(cancelAnimationFrame); };
  }, []);
  ...
  return (
    <section className="relative h-screen min-h-[600px] flex flex-col justify-end pb-24 px-6 md:px-12 overflow-hidden bg-brand-dark">
      <div ref={parallaxRef} className="absolute inset-0 z-0 will-change-transform">
        <Image src="/images/hero/hero-bg.jpg" alt="Yoga Pose" storageKey="hero-main-bg" containerClassName="w-full h-[120%] -top-[10%]" className="w-full h-full object-cover object-center brightness-[0.85]" controlsClassName="top-32 right-6 md:top-36 md:right-8" loading="eager" fetchPriority="high" />
      ...
      <div className="relative z-20 w-full max-w-7xl mx-auto">
        <FadeIn delay={100} direction="down"><Logo className="w-28 h-28 mb-8" color="#fff" /></FadeIn>
        <FadeIn delay={300} direction="up"><h1 className="text-6xl md:text-[8rem] font-serif italic text-white leading-[0.85] mb-8 drop-shadow-lg mix-blend-overlay opacity-90">ЙОГА</h1></FadeIn>
      ...
      <div ref={indicatorRef} className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 block transition-opacity duration-300">
         <div onClick={() => window.scrollTo({ top: window.innerHeight, behavior: 'smooth' })} className="animate-bounce p-3 bg-white/10 backdrop-blur-md rounded-full border border-white/20 text-white/80 hover:bg-white/20 transition-colors cursor-pointer"><ArrowDown className="w-5 h-5" /></div>
      </div>
    </section>
  );
};
