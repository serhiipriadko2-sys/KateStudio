...
import { Upload, X, Link, Check, Loader2, Sparkles, ImageOff, Activity, Zap, Brain } from 'lucide-react';
import { getSavedImageUrl, saveImageMapping, deleteImageMapping, uploadImage } from '../services/content';
...
export const Image: React.FC<ImageProps> = ({ 
        ...
        <div className="space-y-3">
            <div className="flex justify-between items-start border-b border-brand-green/10 pb-2">
                <div>
                    ...
                    <p className="text-xs uppercase tracking-wider text-stone-400 font-bold">{analysisResult.name_ru}</p>
                </div>
                <div className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wide flex items-center gap-1
                    ${analysisResult.energy === 'Brahmana' ? 'bg-amber-100 text-amber-600' : 
                      analysisResult.energy === 'Langhana' ? 'bg-indigo-100 text-indigo-600' : 'bg-emerald-100 text-emerald-600'}`}>
            ...
            <div className="flex flex-wrap gap-1.5">
                {analysisResult.muscles.map((m, i) => (
                    <span key={i} className="flex items-center gap-1 text-[10px] bg-stone-100 text-stone-600 px-2 py-1 rounded-full">
            ...
            <div className="bg-brand-mint/30 p-3 rounded-xl flex gap-2">
                ...
                <p className="text-xs text-brand-text/80 leading-relaxed"><span className="font-bold">Совет:</span> {analysisResult.tips}</p>
            </div>
        </div>
    );
  };
  ...
  return (
    <div className={`relative overflow-hidden bg-stone-100 group ${containerClassName}`}>
      {!hasError && (
        <div className={`absolute inset-0 bg-stone-200 transition-opacity duration-700 z-10 pointer-events-none ${isLoaded ? 'opacity-0' : 'opacity-100'}`}>
                ...
                {isLinkMode ? (
                  <div className="flex items-center gap-2 bg-white/90 backdrop-blur-md p-1.5 rounded-full shadow-lg animate-in fade-in zoom-in duration-200">
                    <input 
                      ...
                      autoFocus
                    />
                    <button onClick={handleLinkSave} className="p-1.5 bg-brand-green text-white rounded-full hover:bg-brand-green/90 transition-colors"><Check className="w-3 h-3" /></button>
                    <button onClick={() => setIsLinkMode(false)} className="p-1.5 bg-stone-200 text-stone-500 rounded-full hover:bg-stone-300 transition-colors"><X className="w-3 h-3" /></button>
                  </div>
                ) : (
                  <>
                    <button onClick={handleAnalyze} disabled={isAnalyzing || isUploading || hasError} className="p-2 bg-white/80 backdrop-blur-md rounded-full hover:bg-brand-accent hover:text-brand-dark text-stone-600 shadow-md transition-colors transform hover:scale-110 disabled:opacity-50" title="Анатомический разбор">
                        {isAnalyzing ? <Loader2 className="w-4 h-4 animate-spin text-brand-green" /> : <Brain className="w-4 h-4" />}
                    </button>
                    <button onClick={triggerUpload} disabled={isUploading} className="p-2 bg-white/80 backdrop-blur-md rounded-full hover:bg-brand-green hover:text-white text-stone-600 shadow-md transition-colors transform hover:scale-110 disabled:opacity-50" title="Загрузить файл"><Upload className="w-4 h-4" /></button>
                    <button onClick={() => setIsLinkMode(true)} className="p-2 bg-white/80 backdrop-blur-md rounded-full hover:bg-brand-green hover:text-white text-stone-600 shadow-md transition-colors transform hover:scale-110" title="Вставить ссылку"><Link className="w-4 h-4" /></button>
                    {src !== defaultSrc && (
                        <button onClick={handleReset} className="p-2 bg-white/80 backdrop-blur-md rounded-full hover:bg-rose-500 hover:text-white text-rose-500 shadow-md transition-colors transform hover:scale-110" title="Сбросить"><X className="w-4 h-4" /></button>
                    )}
                  </>
                )}
            </div>
