...
export const Retreats: React.FC<RetreatsProps> = ({ onBook }) => {
  ...
  const programDays = [
    {
      ...
      desc: 'Встреча в аэропорту Горно-Алтайска. Трансфер на базу. Расселение в эко-домиках. Вечерний круг знакомства у костра и практика настройки.',
    },
    {
      day: 2,
      title: 'Дыхание гор',
      icon: <Sun className="w-5 h-5" />,
      desc: 'Утренняя пранаяма на рассвете. Прогулка к водопаду Чили. Мягкая Хатха-йога для адаптации. Лекция: «Философия йоги в повседневной жизни».',
    },
    {
      day: 3,
      title: 'Поток энергии',
      icon: <Coffee className="w-5 h-5" />,
      desc: 'Активная практика Inside Flow под музыку. Свободное время для чтения и созерцания. Вечерняя медитация под звуки поющих чаш.',
    },
    {
      day: 4,
      title: 'День Тишины (Мауна)',
      icon: <Moon className="w-5 h-5" />,
      desc: 'День без гаджетов и разговоров. Погружение в себя. Практика инь-йоги. Вечером — традиционная баня с травами и выход из молчания.',
    },
    {
      day: 5,
      title: 'Марсианские пейзажи',
      icon: <Camera className="w-5 h-5" />,
      desc: 'Выездная экскурсия на «Марс» — уникальные цветные горы Кызыл-Чина. Фотосессия в летящих платьях. Пикник на природе.',
    },
    {
      day: 6,
      title: 'Единство',
      icon: <Sun className="w-5 h-5" />,
      desc: 'Партнерская йога. Доверительные практики. Прощальный гала-ужин, обмен впечатлениями и подарками.',
    },
    {
      day: 7,
      title: 'Возвращение',
      icon: <Mountain className="w-5 h-5" />,
      desc: 'Финальная утренняя садхана. Завтрак. Трансфер в аэропорт. Объятия и обещание встретиться снова.',
    },
  ];

  const handleBookClick = () => {
    setIsModalOpen(false); // Close the info modal
    onBook('Йога-тур: Сила Тишины (Алтай)'); // Open booking modal
  };

  return (
    <>
      <section className="py-24 px-4 md:px-12 scroll-mt-20">
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 p-8 md:p-16 w-full md:w-2/3 text-white z-20">
                ...
                <p className="text-lg text-white/80 mb-8 leading-relaxed max-w-xl">
                  ...
                  реки, баня и душевные разговоры у костра. Подари себе время быть настоящим.
                </p>
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                  ...
                  <div key={day.day} className="flex gap-4 group">
                    ...
                    <div className="pb-6">
                      <h4 className="text-xl font-medium text-brand-text mb-2 flex items-center gap-2">
                        {day.title}
                        <span className="text-stone-300 scale-75 opacity-0 group-hover:opacity-100 transition-opacity">
                          {day.icon}
                        </span>
                      </h4>
                      <p className="text-brand-text/70 text-sm leading-relaxed">{day.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
