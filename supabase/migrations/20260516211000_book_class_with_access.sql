-- Server-side booking gate for app classes.
-- First iteration: move booking creation behind one RPC and enforce active pass presence.

drop function if exists public.book_class_with_access(text, text, date, text, text, bigint);

create or replace function public.book_class_with_access(
  p_class_id text,
  p_class_name text,
  p_class_date date,
  p_class_time text,
...
revoke all on function public.book_class_with_access(text, text, date, text, text, bigint) from anon;
