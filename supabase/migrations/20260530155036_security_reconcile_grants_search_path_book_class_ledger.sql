-- Security reconciliation forward migration.
--
-- Purpose:
-- 1. Re-play the idempotent book-class-with-access hardening represented in Git by
--    supabase/migrations/20260528213234_book_class_edge_private_helper.sql so live
--    has a current migration-ledger receipt for the already-deployed behavior.
-- 2. Remove excess anon/authenticated table privileges from payment_orders and user_passes.
-- 3. Fix Supabase advisor WARNs for functions without an explicit search_path.
...
revoke insert, update, delete, truncate, references, trigger on table public.payment_orders from authenticated;
revoke insert, update, delete, truncate, references, trigger on table public.user_passes from authenticated;

grant select on table public.payment_orders to authenticated;
grant select on table public.user_passes to authenticated;

alter function public.graph_find_resonant(real, integer) set search_path = public;
alter function public.graph_get_node_with_edges(text) set search_path = public;
alter function public.graph_bfs_traversal(text, integer, real) set search_path = public;
alter function public.update_updated_at() set search_path = public;
alter function public.update_graph_nodes_updated_at() set search_path = public;
